...
class WSServer:
    ...
    def _handle_message(self, client: dict, data: dict) -> None:
        if not client['username']:
            return
        if client['muted']:
            return self._send_error(client, 'muted', 'You have been muted by a moderator.')
        if not client['bucket'].try_consume(1):
            return self._send_error(client, 'rate_limited', 'Sending too fast — slow down a little.')
